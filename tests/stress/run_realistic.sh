...
if [ -f .env ]; then
    ...
    echo "ERROR: .env not found — real credentials required"
    exit 1
fi
...
BOT_PORT=8000
PHASES=(5 10 15 20 30 40)
PHASE_DURATION=90  # seconds per phase
RESULTS_FILE="tests/stress/results_realistic_$(date +%Y%m%d_%H%M%S).txt"

# ── Install dependencies ───────────────────────────────────────────────────────
echo "[run_realistic.sh] Installing locust and psutil..."
pip install --quiet locust psutil

# ── Start bot in stress mode ───────────────────────────────────────────────────
echo "[run_realistic.sh] Starting bot with STRESS_MODE=1 STRESS_REALISTIC=1..."
LOG_LEVEL=WARNING STRESS_MODE=1 STRESS_REALISTIC=1 uvicorn app.main:app \
    --port "$BOT_PORT" --log-level warning \
    > /tmp/stress_realistic_bot.log 2>&1 &
BOT_PID=$!
echo "[run_realistic.sh] Bot PID: $BOT_PID"

# ── Health check ──────────────────────────────────────────────────────────────
echo "[run_realistic.sh] Waiting for bot to be ready..."
for i in $(seq 1 30); do
    if curl -sf "$HOST/health" > /dev/null 2>&1; then
        echo "[run_realistic.sh] Bot is ready."
        break
    fi
    if [ "$i" -eq 30 ]; then
        echo "[run_realistic.sh] ERROR: Bot did not become ready in time."
...
")
echo "[run_realistic.sh] Baseline RSS: ${BASELINE_RSS} MB"
echo "[run_realistic.sh] Saving results to: $RESULTS_FILE"

# ── Intro ─────────────────────────────────────────────────────────────────────
{
printf "Stress Test — Escenario 2: Latencia real de Calendar\n"
printf "Modo:        Calendar lecturas reales, crear_cita mockeado, WhatsApp mockeado\n"
printf "Fecha:       %s\n" "$(date '+%Y-%m-%d %H:%M:%S')"
...
for USERS in "${PHASES[@]}"; do
    echo "[run_realistic.sh] Phase: $USERS users for ${PHASE_DURATION}s..."
...
echo "[run_realistic.sh] Results saved to: $RESULTS_FILE"

# ── Teardown ──────────────────────────────────────────────────────────────────
echo "[run_realistic.sh] Killing bot (PID $BOT_PID)..."
kill "$BOT_PID" 2>/dev/null || true
wait "$BOT_PID" 2>/dev/null || true
echo "[run_realistic.sh] Done."
