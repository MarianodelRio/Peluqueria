...
def _make_manual_event(event_id: str, telefono: str = "34600000001", nombre: str = "Ana"):
    """Build a minimal manual event dict as returned by get_eventos_manuales_sin_confirmar."""
...
class TestJobSyncCitasManuales:
    ...
    def test_processes_events_without_refetching(self, monkeypatch):
        """2 pending manual events → send_template x2, marcar_manual_confirmado x2, _get_service never called."""
        monkeypatch.setattr("app.services.scheduler.ENVIAR_CONFIRMACIONES", True)
        events = [_make_manual_event("evt1", "34600000001"), _make_manual_event("evt2", "34600000002")]
        mock_get_service = MagicMock()
        with patch("app.services.scheduler.cal_service.get_eventos_manuales_sin_confirmar", return_value=events), \
             patch("app.services.scheduler.wa_service.send_template", return_value=True) as mock_send, \
             patch("app.services.scheduler.cal_service.marcar_manual_confirmado", return_value=True) as mock_mark, \
