...
from app.config import GOOGLE_CREDENTIALS_PATH, GOOGLE_CALENDAR_ID, TIMEZONE, CITA_DURACION_MIN
...
def _get_service():
    ...
    if not hasattr(_thread_local, 'service'):
        ...
        _thread_local.service = build('calendar', 'v3', credentials=creds, cache_discovery=False)
...
def get_slots_disponibles(d: date) -> List[str]:
    """
    Returns available slots for a given date.
    Single pass over events: detect [CFG] closures, special schedule, and blocking events.
    """
...
def slot_sigue_libre(d: date, hora: str) -> bool:
    ...
    Called immediately before creating an appointment (anti-concurrency).
    """
    available = get_slots_disponibles(d)
...
def get_citas_futuras(telefono: str) -> list:
    ...
    Returns list of dicts: {id, title, description, start, end}
    """
    service = _get_service()
    now = datetime.now(TZ)
...
def check_calendar_health() -> bool:
    """
    Verify Google Calendar connectivity by fetching the calendar metadata.
